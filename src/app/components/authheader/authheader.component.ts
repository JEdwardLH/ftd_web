  ...
  getCartDetails() {
    ...
    this.CartService.getCart(body, headers).subscribe((data: any) => {
      if (data.code == 200) {
        ...
        this.cartcount = data.data.total_cart_count
      } else {
        this.cartcount = 0;
      }
...
@Component({
  selector: 'authheader.componentsdialog',
  templateUrl: 'authheader.dialog.html',
})
export class DialogOverviewExampleDialog {
  message: any
  constructor(
    public dialogRef: MatDialogRef<DialogOverviewExampleDialog>,
    @Inject(MAT_DIALOG_DATA) public data: AuthheaderComponent) {
